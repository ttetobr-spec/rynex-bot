...
slashCommands.push(new SlashCommandBuilder().setName('produto').setDescription('Cadastrar ou atualizar um produto oficial').addStringOption(o => o.setName('nome').setDescription('Nome do produto').setRequired(true)).addStringOption(o => o.setName('preco').setDescription('Preço, por exemplo R$ 14').setRequired(true)).addStringOption(o => o.setName('informacao').setDescription('Informação adicional').setRequired(false)).toJSON());
slashCommands.push(new SlashCommandBuilder().setName('resposta').setDescription('Enviar ou cadastrar uma resposta rápida').addStringOption(o => o.setName('nome').setDescription('Nome da resposta').setRequired(true)).addStringOption(o => o.setName('texto').setDescription('Texto; preenchido pela equipe para salvar').setRequired(false)).toJSON());
slashCommands.push(new SlashCommandBuilder().setName('pedido').setDescription('Abrir um ticket privado de pedido').toJSON());
slashCommands.push(new SlashCommandBuilder().setName('limpar').setDescription('Apagar mensagens recentes').addIntegerOption(o => o.setName('quantidade').setDescription('Entre 1 e 100').setMinValue(1).setMaxValue(100).setRequired(true)).toJSON());
slashCommands.push(new SlashCommandBuilder().setName('relatorio').setDescription('Ver estatísticas do servidor').toJSON());
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    if (interaction.commandName === 'produto') {
      if (!admin) return interaction.reply({ content: 'Somente a equipe autorizada pode alterar o catálogo.', ephemeral: true });
      const name = interaction.options.getString('nome').trim().toLowerCase(); cfg.products[name] = { price: interaction.options.getString('preco').trim(), info: interaction.options.getString('informacao')?.trim() || '', updatedBy: interaction.user.id, updatedAt: new Date().toISOString() }; save();
      return interaction.reply({ content: `✅ Produto **${name}** salvo no catálogo oficial.`, ephemeral: true });
    }
    if (interaction.commandName === 'resposta') {
      const name = interaction.options.getString('nome').trim().toLowerCase(); const text = interaction.options.getString('texto')?.trim();
      if (text) { if (!admin) return interaction.reply({ content: 'Somente a equipe autorizada pode cadastrar respostas rápidas.', ephemeral: true }); cfg.quickReplies[name] = text; save(); return interaction.reply({ content: `✅ Resposta rápida **${name}** salva.`, ephemeral: true }); }
      const response = cfg.quickReplies[name];
      return interaction.reply({ content: response || `Não existe uma resposta rápida chamada **${name}**.`, ephemeral: true });
    }
    if (interaction.commandName === 'pedido') return openTicket(interaction, cfg, 'pedido');
    if (interaction.commandName === 'limpar') {
      if (!admin || !interaction.member.permissions.has(PermissionsBitField.Flags.ManageMessages)) return interaction.reply({ content: 'Somente a equipe com permissão de gerenciar mensagens pode usar este comando.', ephemeral: true });
      const amount = interaction.options.getInteger('quantidade'); await interaction.channel.bulkDelete(amount, true); return interaction.reply({ content: `🧹 ${amount} mensagens removidas.`, ephemeral: true });
    }
    if (interaction.commandName === 'relatorio') { if (!admin) return interaction.reply({ content: 'Somente a equipe autorizada pode ver o relatório.', ephemeral: true }); return interaction.reply({ embeds: [reportEmbed(cfg)], ephemeral: true }); }
    return;
  }
